...
5. When you are ready to merge to `master`, make sure your branch has
   ...
   label.
6. If appropriate, use the GitHub "Reviewers" dropdown to formally
...
7. At least one other person must review any changes to the `master`
...
8. Before it can be merged, you will generally have to `rebase` your
...
9. Note that *force-pushes can be dangerous*, so make sure that you know
...
10. Finally, a _reviewer_ with merge permissions can merge the PR using the
   GitLab "Merge pull request" button.
   This will introduce an _unsigned_ merge commit, but
   preserve the signatures, if any, on the actual branch's commits. Finally,
